...
- 10万件 HNSW (index_granularity=8192 デフォルト): 検索 p50 45.4ms、recall@10 0.926
- 10万件 HNSW (index_granularity=128 チューニング後): **p50 10.6ms**、recall@10 0.918。スケールしてもほぼ横ばい（1万件 9.0ms → 10万件 10.6ms）
- 10万件 brute force（インデックスなし）: p50 56.2ms、recall@10 0.952（厳密検索 = 品質の天井）
- 書き込みは速い: 10万件で取り込み 10-12秒 + OPTIMIZE FINAL 8秒
- vector_similarity インデックスは 25.x 時点で experimental

## 選定の目安

- 既に ClickHouse で分析基盤を持っているなら、ベクトル検索を同居させる選択肢として現実的
- 10万件規模なら brute force ですら 56ms — 「インデックスなしで厳密検索」が許される規模は意外と広い
...
- index_granularity 8192 → 128 で p50 45.4ms → 10.6ms（4.3倍）
- ただし recall も 0.926 → 0.918 に微減する。大きい granule は再スコアリング対象が
  多く（10 granule × 8192行 ≒ 表の8割）準 brute force として recall を押し上げていた
- つまり「遅さの正体」と「recall の源泉」が同じ設定に埋まっている
