...
1万件で 0.976〜0.983 の統計的同着。「どの DB が賢いか」はほぼ無意味な問いで、
差が出るのは (1) レイテンシの固定費構造 (2) 書き込み側の速さ (3) 運用の罠の数、
の3つ。

例外は量子化を既定で挟むもの（LanceDB IVF_HNSW_SQ: 1万件 0.901 / 10万件 0.811）。
「HNSW と書いてあるか」ではなく「量子化が入るか」を見る。

## 規模の目安

- 〜10万件: brute force（厳密検索）ですら ClickHouse で p50 66ms。
  レイテンシ要件が緩ければインデックスなし=recall 満点という選択肢が残る
- 10万件で HNSW 勢の recall@10 は 0.921〜0.947、厳密検索が 0.952。
  近似のコストは 0.5〜3.1 ポイント (Qdrant 0.947 は天井まで 0.5pt)
- コーパスが大きくなると全員 recall が下がる（1万件 0.98 帯 → 10万件 0.92-0.95 帯）。
...
10万件の取り込み+インデックス構築: ClickHouse 19-20秒、pgvector 56秒。
夜間バッチでコーパスを作り直す設計なら、この 2.7 倍差は検索 p50 の数 ms 差より
